...
api.get('/proveedor/:id?', function (req, res) {
    ...
    if (req.params.id) {//si existe id en el URL, se selecciona las partes que surte el cliente
        ...
        sql = `select a.no_parte interior,a.no_parte_ext exterior,a.descripcion descripcion,a.cant_x_caja caja,a.cant_x_pallet pallet,a.existencia,a.cant_min from partes a inner join (select id_cliente,nombre from clientes where id_cliente=${id_cliente}) b on a.id_proveedor=b.id_cliente;`;
        con.query(sql, function (err, rows) {
            if (err) throw err
            else res.send(rows);
        });
    } else {//Partes que surte el cliente.
        sql = `select b.nombre, a.no_parte, a.descripcion from partes a inner join clientes b on a.id_proveedor=b.id_cliente;`;
        con.query(sql, function (err, rows) {
            if (err) throw err
            else res.send(rows);
        });
